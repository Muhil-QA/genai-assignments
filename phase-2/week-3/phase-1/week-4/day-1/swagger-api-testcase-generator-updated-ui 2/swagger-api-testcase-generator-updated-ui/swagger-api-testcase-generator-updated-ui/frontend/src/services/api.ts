...
async function parse<T>(response: Response): Promise<T> {
  ...
  if (!response.ok || !payload.success) {
    ...
    const details = error.error?.details as { issues?: Array<{ path?: (string | number)[]; message?: string }> } | undefined;
    const firstIssue = details?.issues?.[0];
    const issueText = firstIssue?.path?.length && firstIssue.message
      ? ` (${firstIssue.path.join(".")}: ${firstIssue.message})`
      : "";
    throw new Error(`${error.error?.message || "Request failed."}${issueText}`);
  }
  return payload.data;
}
...
export async function uploadSwaggerLink(url: string) {
  return parse<{ sessionId: string; fileName: string; format: string; validationReport: ValidationReport }>(
    await fetch(`${BASE}/swagger/upload-link`, {
      method: "POST",
