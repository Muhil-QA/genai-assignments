...
import { CorrectSwaggerRequestSchema, SessionRequestSchema, UploadSwaggerLinkRequestSchema } from "../schemas/api.schemas.js";
import { correctSpecification } from "../services/correction.service.js";
import { fetchSpecification } from "../services/swagger-fetch.service.js";
...
export const uploadSwaggerLink: RequestHandler = async (req, res) => {
  const { url } = UploadSwaggerLinkRequestSchema.parse(req.body);
  const remote = await fetchSpecification(url);
  const parsed = parseSpecification(remote.fileName, remote.content);
  const validationReport = await validateSpecification(parsed.specification);
  const sessionId = randomUUID();
  const now = new Date();
  ...
  res.status(201).json({
    success: true,
    data: { sessionId, fileName: remote.fileName, format: parsed.format, status: "uploaded", validationReport }
  });
};
