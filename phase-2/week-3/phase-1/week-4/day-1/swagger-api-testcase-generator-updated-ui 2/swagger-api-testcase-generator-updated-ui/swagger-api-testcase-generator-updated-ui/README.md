...
Upload an OpenAPI/Swagger document, validate it, auto-correct it, generate API test cases with optional Groq LLM support, automatically classify each case with multiple standard QA types, and export the results as JSON, Excel, a corrected Swagger file, or a Postman collection.

Generated cases retain their existing scenario `category` (such as `positive`, `negative`, or `validation`) and additionally receive `standardTestTypes`. Standard types include Smoke, Sanity, Regression, Functional, Integration, End-to-End, API/Contract, Validation, Negative, Boundary, Security, Performance, Compatibility, Usability, and Exploratory when supported by the testcase evidence.

**Stack:** React 19 + Vite (frontend) → Express 5 + TypeScript (backend) → Groq LLM API (`openai/v1/chat/completions`-compatible endpoint), all in-memory (no database).
...
| [components/UploadPanel.tsx](frontend/src/components/UploadPanel.tsx) | Step 1 file UI. Drag-and-drop or file-picker for `.yaml/.yml/.json`. Calls `onUpload(file)` → `App.handleUpload` → `api.uploadSwagger`. |
| [components/InputModePanel.tsx](frontend/src/components/InputModePanel.tsx) | Step 1 input-source selector for local file or remote link. |
| [components/UploadLinkPanel.tsx](frontend/src/components/UploadLinkPanel.tsx) | Step 1 URL UI. Accepts an HTTP or HTTPS specification URL and calls `App.handleUploadLink`. |
...
2. `UploadLinkPanel` → `App.handleUploadLink(url)` → `api.uploadSwaggerLink(url)` → `POST /api/v1/swagger/upload-link`
3. `ValidationPanel` → `App.handleCorrect()` → `api.correctSwagger(sessionId)` → `POST /api/v1/swagger/correct`
...
| [config/environment.ts](backend/src/config/environment.ts) | Loads `.env` via `dotenv`, validates `process.env` with a Zod schema (`PORT`, `CORS_ORIGIN`, `MAX_FILE_SIZE_MB`, `SESSION_TTL_MINUTES`, `GROQ_API_KEY`, `GROQ_MODEL`, `GROQ_TIMEOUT_MS`, `MAX_TEST_CASES`), and exports `groqEnabled` (`true` only when both `GROQ_API_KEY` and `GROQ_MODEL` are set). This flag is the single switch that determines whether Groq is ever called. |

### Endpoint → Controller → Service map

| Method & Path | Controller | Service(s) called | Purpose |
|---|---|---|---|
| `GET /health` | inline in `routes/index.ts` | — | Liveness + reports `groqConfigured`. |
...
| `POST /swagger/upload-link` | [swagger.controller.ts](backend/src/controllers/swagger.controller.ts) `uploadSwaggerLink` | `swagger-fetch.service.fetchSpecification`, `swagger.service.parseSpecification`, `.validateSpecification` | Fetches a bounded remote YAML/JSON document over HTTP(S), validates it, and creates a new session. |
...
| `POST /swagger/correct` | `swagger.controller.ts` `correctSwagger` | `correction.service.correctSpecification` (→ `groq.service` if needed) | Deterministically fixes common issues, then (unless `applySafeCorrectionsOnly`) asks Groq to repair remaining errors. |
...
| `POST /testcases/generate` | `testcase.controller.ts` `generateCases` | `testcase.service.generateTestCases` (→ `groq.service`) | Core generation: deterministic engine + optional Groq augmentation. |
...
| [groq.service.ts](backend/src/services/groq.service.ts) | **The only file that talks to Groq.** `isGroqEnabled()` mirrors `config/environment.groqEnabled`. `callGroqJson(systemPrompt, userPrompt)` POSTs to `https://api.groq.com/openai/v1/chat/completions` with `model: env.GROQ_MODEL`, `temperature: 0.1`, `response_format: {type:"json_object"}`, and the two messages; enforces `GROQ_TIMEOUT_MS` via `AbortController`; parses `choices[0].message.content` as JSON; wraps every failure mode (non-OK response, empty content, invalid JSON, timeout, network error) in a typed `AppError` so callers can catch/fallback uniformly. |
...
## 5. Where Groq fits in

Groq is called from exactly **one** module — [groq.service.ts](backend/src/services/groq.service.ts) — and only by two callers:
...
Both callers:
- Skip Groq entirely if `groqEnabled` is `false` (no `GROQ_API_KEY`/`GROQ_MODEL` in `.env`) — the app is fully functional without Groq, using deterministic generation/correction only.
- Validate every Groq JSON response with a Zod schema in [llm.schemas.ts](backend/src/schemas/llm.schemas.ts) before trusting it.
- Catch any Groq failure (timeout, bad JSON, HTTP error) and fall back to the deterministic result rather than failing the request.
...
# Backend
cd backend
# create a .env with PORT, CORS_ORIGIN, and (optionally) GROQ_API_KEY + GROQ_MODEL to enable LLM features
npm install
