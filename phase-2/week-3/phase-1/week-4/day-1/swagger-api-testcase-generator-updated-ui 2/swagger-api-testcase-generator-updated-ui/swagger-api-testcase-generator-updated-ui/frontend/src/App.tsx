...
import { UploadLinkPanel } from "./components/UploadLinkPanel";
...
import { clearSession, confirmGeneration, correctSwagger, downloadUrl, generatePostman, generateTestCases, uploadSwagger, uploadSwaggerLink } from "./services/api";
...
export default function App() {
  ...
  const handleUploadLink = async (url: string) => {
    ...
    setNotice(undefined);
    try {
      const data = await uploadSwaggerLink(url);
      setSessionId(data.sessionId);
      setFileName(data.fileName);
      setValidation(data.validationReport);
      setCorrection(undefined);
      setTestCases([]);
      setSummary(undefined);
      setNotice({ type: data.validationReport.isValid ? "success" : "error", text: data.validationReport.isValid ? "Swagger link fetched and validated successfully." : "Swagger link fetched. Review and correct the validation errors." });
    } catch (error) {
      setNotice({ type: "error", text: error instanceof Error ? error.message : "Link import failed." });
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setNotice({ type: "success", text: `${result.summary.totalTestCases} API testcases generated.` });
    } catch (error) {
      ...
      <main>
        <section className="hero-section">
          <div className="hero-copy">
            ...
            <p>Upload, validate, generate, review, and export — through one controlled Testleaf-themed workflow.</p>
          ...
          <ol className="flow-rail" aria-label="Application flow">
            {[
              ["01", "Upload"], ["02", "Validate"], ["03", "Generate"], ["04", "Export"]
        ...
        {!sessionId && (
          <>
            <InputModePanel mode={inputMode} onChange={(mode) => { setInputMode(mode); setNotice(undefined); }} />
            {inputMode === "file" && <UploadPanel busy={busy} onUpload={handleUpload} onClear={handleClear} visible={!!sessionId} />}
            {inputMode === "link" && <UploadLinkPanel busy={busy} onUploadLink={handleUploadLink} onClear={handleClear} visible={!!sessionId} />}
          </>
        )}
